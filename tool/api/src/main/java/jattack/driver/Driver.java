...
public class Driver {
    ...
    private static void executeEntryMethod0(Object receiver, Object... args)
            throws InvocationTargetException, IllegalAccessException {
        try {
            if (!entryMethodIsStatic && receiver == null) {
                if (Config.mimicExecution) {
                    checksum.update(NullPointerException.class.getName());
                }
                return;
            }
    ...
    private static void handleExceptionInInvocation(InvocationTargetException e)
        ...
        // We ignore exceptions that is caused by template itself not
        // jattack.
        if (TypeUtil.isToolOwnClass(frame.getClassName()) && !isTemplateException(cause)) {
            throw e;
        } else {
            // Ignore
            ...
            // Perform the same checksum as we do in main
            // generated by output transformer for testing
            // purpose
            if (Config.mimicExecution) {
                if (isTemplateException(cause)) {
                    // Get the actual exception wraped in our custom
                    // exception
    ...
    private static boolean isTemplateException(Throwable e) {
        return e instanceof InvocationTemplateException;
    }
