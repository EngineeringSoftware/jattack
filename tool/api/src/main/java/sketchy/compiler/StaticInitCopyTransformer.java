...
import sketchy.bytecode.StaticInitRenamer;
...
public class StaticInitCopyTransformer implements ClassFileTransformer {
    ...
    public byte[] transform(
        ...
        try {
            ...
            // DEBUGGING
            // try {
            //     sketchy.util.IOUtil.saveBytecodeToFile(
    ...
    private boolean isIgnoreClass(String className) {
        String sketchClassName = TypeUtil.bin2Intern(Config.sketchClzFullName);
        return !className.equals(sketchClassName)
                && !className.startsWith(sketchClassName + '$');
    }
}
